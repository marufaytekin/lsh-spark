...
/** Create LSH model for maximum m number of elements in each vector.
  ...
  *
  * */
class LSHModel(val m: Int, val numHashFunc : Int, val numBands: Int)
...
object LSHModel {
  ...
  private [lsh] object SaveLoadV0_0_1 {
    ...
    def load(sc: SparkContext, path: String): LSHModel = {
      ...
      val numBands = bands.map(x => x._1._1).distinct.count()
      val numHashFunc = hashers.size / numBands

      //Validate loaded data
      //check size of data
      assert(bands.count != 0, s"Loaded band data is empty")
      ...
      assert(bands.map(x => x._1._2).filter(x => x.size != numHashFunc).collect().size == 0,
